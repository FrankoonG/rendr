...
var tier3GoTestExecutor = gotestjson.Executor{
...
func goTestReportCases(defs []caseDef, result gotestjson.Result) []report.Case {
	if len(defs) == 0 {
		return nil
	}
	owner := make(map[string]int)
	for i, def := range defs {
		for _, name := range def.expected {
			owner[name] = i
		}
	}

	tests := make([][]gotestjson.TestResult, len(defs))
	issues := make([][]gotestjson.Issue, len(defs))
	for _, testResult := range result.Tests {
		index, ok := owner[testResult.Name]
		if !ok {
			index = 0
		}
		tests[index] = append(tests[index], testResult)
	}
	for _, issue := range result.Issues {
		index, ok := owner[issue.Test]
		if !ok {
			index = 0
		}
		issues[index] = append(issues[index], issue)
	}

	cases := make([]report.Case, len(defs))
	for i, def := range defs {
		subresult := gotestjson.Result{Tests: tests[i], Issues: issues[i]}
		for _, testResult := range tests[i] {
			subresult.Duration += testResult.Duration
		}
		if i == 0 {
			subresult.Command = result.Command
			subresult.CommandOutput = result.CommandOutput
			subresult.CommandOutputTruncated = result.CommandOutputTruncated
			subresult.CaptureTruncated = result.CaptureTruncated
		}
		cases[i] = goTestReportCase(def.spec.ID, def.spec.Tier, def.expected, subresult)
	}
	return cases
}
