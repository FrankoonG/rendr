...
// TestT3VlessVisionTLSxItself — PYS-D-1 reference per
// docs/regression-suite.md §7.2.1: VLESS + Vision flow + TLS.
// Self-signed cert (CN=localhost) pinned by sha256 on the client
// side so the 127.0.0.1 vs CN-localhost hostname mismatch is
// bypassed. Both paths share the same UUID + same xtls-rprx-vision
// flow + same TLS pin; each is an independent VLESS session.
//
// This is the doc's canonical "first reference" path-profile and
// the one xray's own modern guidance steers users toward (VLESS
// without flow is deprecated).
func TestT3VlessVisionTLSxItself(t *testing.T) {
	...
	r := driver.RunFileXfer(ctx, driver.FileXferOpts{
		CaseName: "T3.stream.vless+vision+tls × itself",
...
func startVlessVisionServer(t *testing.T, port int, userID string, srvCert *cert.Certificate) *core.Instance {
	t.Helper()
	cfg := &core.Config{
...
func startVlessVisionClient(t *testing.T, port int, userID string, srvCertHash [32]byte) *core.Instance {
	t.Helper()
	cfg := &core.Config{
		App: []*serial.TypedMessage{
			serial.ToTypedMessage(&dispatcher.Config{}),
