...
func TestT3StreamXrayBalancerFreedom(t *testing.T) {
	inst := startTaggedFreedomInstance(t, "stream-path-a", "stream-path-b")
	defer inst.Close()

	factories, err := rendrxray.XrayBalancerAsStreamFactories(inst, &xrouter.BalancingRule{
		Tag:              "rendr-stream-paths",
		OutboundSelector: []string{"stream-path-"},
		Strategy:         "roundrobin",
	})
	if err != nil {
		t.Fatal(err)
	...
	paths := make([]rendr.PathSpec, 0, len(factories))
	named := make([]driver.NamedFactory, 0, len(factories))
	for _, f := range factories {
		paths = append(paths, rendr.PathSpec{Transport: f.Name})
		named = append(named, driver.NamedFactory{Name: f.Name, Stream: f.Factory})
	}
	...
	r := driver.RunFileXfer(ctx, driver.FileXferOpts{
		CaseName:  "T3.stream.xray-balancer-freedom",
		Paths:     paths,
		Factories: named,
	})
	if r.Failure != "" {
